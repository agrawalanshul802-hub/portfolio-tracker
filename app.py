    ...
    try:
        with sqlite3.connect(DATABASE) as conn:
            ...
            if row:
                ...
                # If user signed in with Google, register them if not in DB
                import uuid
...
def run_local_analysis(message, holdings):
    ...
    if msg_lower in ['hello', 'hi', 'hey', 'help', 'who are you', 'what can you do', 'good morning', 'good evening']:
        if not holdings:
            return """### 👋 Hello! I am your AI Investment Analyst.
...
* Once you add holdings, ask me questions about **diversification**, **risk profile**, **sector allocation**, or **performance**!"""
            ...
            return f"""### 👋 Hello! I am your AI Investment Analyst.
...
* **Review Laggards**: Periodically assess underperformers like {highest_loser.get('symbol')} to verify fundamental strength."""
    ...
    # 4. Default portfolio overview response
        ...
        gain_sign = "+" if total_gain >= 0 else ""
        top_pct = (largest_holding_val / safe_total_value * 100)
        return f"""### 👋 Portfolio AI Analyst Overview
...
* **Largest Position**: **{largest_holding.get('symbol')}** (₹{largest_holding_val:,.2f}, representing {top_pct:.1f}%)
...
def generate_fallback_analysis(holdings_summary, total_val, total_inv, total_pnl, total_pnl_pct, message):
    ...
    lines.append(f"- **Total Valuation:** ₹{total_val:,.2f}")
    lines.append(f"- **Total Capital Invested:** ₹{total_inv:,.2f}")
    ...
    if "risk" in msg_lower:
        lines.append("### 🛡️ Risk Profile & Concentration Analysis")
        if len(holdings_summary) <= 3:
            lines.append("⚠️ **High Concentration Risk:** You hold 3 or fewer assets. A sharp drop in any single stock will significantly impact total net worth.")
        else:
            lines.append("✅ **Moderate Diversification:** Your capital is spread across multiple holdings, reducing single-stock volatility.")
        if top_winner:
        ...
        lines.append("### 💡 Strategic Portfolio Recommendations")
        ...
        lines.append("### 📈 Asset Performance Breakdown")
        for h in sorted_holdings[:5]:
            pnl_badge = f"+₹{h['gain']:,.2f} (+{h['gainPct']:.2f}%)" if h['gain'] >= 0 else f"-₹{abs(h['gain']):,.2f} ({h['gainPct']:.2f}%)"
            lines.append(f"- **{h['symbol']}**: Invested ₹{h['buyPrice'] * h['qty']:,.2f} → Value ₹{h['value']:,.2f} | **{pnl_badge}**")
        lines.append("")
...
def get_ipo_detail():
    ...
    for slug in unique_slugs:
        ...
        try:
            ...
            issue_size_cr = f"₹{raw_size / 10000000:.2f} Cr" if raw_size else "TBA"
            ...
            'categories':           [
                {
                    ...
                    'categorySubText': 'Apply upto ₹2,00,000',
                ...
                },
                {
                    ...
                    'categorySubText': 'Apply between ₹2,00,000 - ₹5,00,000+',
