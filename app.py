...
# -------------------------------------------------------------
# REST API: Indian Market Mood Index (MMI) & Live Major Indices
# -------------------------------------------------------------
MARKET_MOOD_CACHE = {'timestamp': 0, 'data': None}
MARKET_MOOD_TTL = 60  # 60-second cache

@app.route('/api/market-mood', methods=['GET'])
def get_market_mood():
    current_time = time.time()
    if MARKET_MOOD_CACHE['data'] and (current_time - MARKET_MOOD_CACHE['timestamp'] < MARKET_MOOD_TTL):
        return jsonify(MARKET_MOOD_CACHE['data'])

    indices = [
        {'id': 'NIFTY50', 'name': 'NIFTY 50', 'symbol': '^NSEI'},
        {'id': 'SENSEX', 'name': 'SENSEX', 'symbol': '^BSESN'},
        {'id': 'BANKNIFTY', 'name': 'BANK NIFTY', 'symbol': '^NSEBANK'},
        {'id': 'INDIAVIX', 'name': 'INDIA VIX', 'symbol': '^INDIAVIX'}
    ]
    ...
    for idx in indices:
        ...
        try:
            ...
            # Fallbacks for indices if network is down
            fallback_map = {
                'NIFTY50': {'name': 'NIFTY 50', 'price': 24350.50, 'prevClose': 24280.00, 'change': 70.50, 'changePct': 0.29},
                'SENSEX': {'name': 'SENSEX', 'price': 80105.20, 'prevClose': 79890.00, 'change': 215.20, 'changePct': 0.27},
                'BANKNIFTY': {'name': 'BANK NIFTY', 'price': 51240.00, 'prevClose': 51100.00, 'change': 140.00, 'changePct': 0.27},
                'INDIAVIX': {'name': 'INDIA VIX', 'price': 13.80, 'prevClose': 14.10, 'change': -0.30, 'changePct': -2.13}
            }
            results[idx['id']] = fallback_map.get(idx['id'])

    # Compute Market Mood Index (0 to 100)
    nifty = results.get('NIFTY50', {})
    vix = results.get('INDIAVIX', {})
    
    nifty_pct = nifty.get('changePct', 0)
    vix_val = vix.get('price', 14.0)

    # VIX baseline ~15; lower VIX -> higher greed, high VIX (>20) -> extreme fear
    vix_score = max(0, min(100, 100 - (vix_val - 10) * 5))
    momentum_score = max(0, min(100, 50 + nifty_pct * 15))
    
    mmi_score = round(0.6 * vix_score + 0.4 * momentum_score, 1)
    
    if mmi_score >= 70:
        sentiment = "Extreme Greed"
        color = "#10B981"
        desc = "Market is heavily bullish & euphoric. Tighten trailing stop-losses."
    elif mmi_score >= 50:
        sentiment = "Greed"
        color = "#34D399"
        desc = "Positive sentiment. Favorable market conditions across major sectors."
    elif mmi_score >= 35:
        sentiment = "Neutral / Fear"
        color = "#FBBF24"
        desc = "Balanced market. Good accumulation zone for quality large-caps."
    else:
        sentiment = "Extreme Fear"
        color = "#EF4444"
        desc = "High market panic / volatility. Excellent long-term buying opportunity."
    ...
    MARKET_MOOD_CACHE['timestamp'] = current_time
    MARKET_MOOD_CACHE['data'] = payload
